...
def main(**kwargs):
    ...
    selected_samples = [0] * 100
    i = 0
    count = 0
    for data, label in test_loader:
        for d, l in zip(data, label):
            if selected_samples[l] < 100 and count < 100:
                if int(l.item()) == i:
                    first_test_batch.append(d)
                    first_test_batch_label.append(l)
                    selected_samples[l] += 1
                    count += 1
                    if count % 10 == 0:
                        i += 1

    first_test_batch = torch.stack(first_test_batch)
    first_test_batch_label = torch.tensor(first_test_batch_label)
    
    first_test_batch, first_test_batch_label = first_test_batch.to(device), first_test_batch_label.to(device)
    

    summary(autoencoder.encoder, (3, 32, 42))
    summary(autoencoder.decoder, (20,))
